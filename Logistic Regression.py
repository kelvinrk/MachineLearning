...
accuracy = 0.0
lr = sl.LogisticRegression( penalty='l1', tol=0.01)
...
print "Mean accuracy of Logistic Regression Predictions on the data was: " + str(accuracy/10)
...
XT = np.asarray(xt)
res_lr = lr.fit(X, y).predict(XT)
res_lr = DataFrame(res_lr,columns=['Survived'])
res_lr.to_csv("Logistic Regression Result.csv") 

##*******************************Train data Accuracy*******************************
train_prediction = lr.fit(X, y).predict(X)
result_train = lr.score(X, y)
print "Accuracy of Logistic Regression Predictions on the Training data was: " + str(result_train)
...
test_prediction = lr.fit(X_train, y_train).predict(X_test)
right = sum(test_prediction==y_test)
result_test = 1.0*right/len(X_test)
print "Accuracy of Logistic Regression Predictions on the Test data was: " + str(result_test)
